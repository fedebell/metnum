...
int clusterize(int*** latt, int*** cluster, int boundary, unsigned int l , unsigned int t, long double beta, unsigned int i, unsigned int j, unsigned int k);
...
int clusterize(int*** latt, int*** cluster, int boundary, unsigned int l , unsigned int t, long double beta, unsigned int i, unsigned int j, unsigned int k) {
	...
	//flag indica se il cluster si avvolge su se stesso
	random_device rd;
    	mt19937 mt(rd());
	...
	for(int d = 0; d < 3; d++) {
		for(int a = -1; a < 2; a = a + 2) {
			...
			//attenzione: Se c'è un cross poi esplora i dintorni e scatta il flag.
			if (distribution(mt) < p) {
				if(!cross) { if((cluster[coord[0]][coord[1]][coord[2]] * cluster[i][j][k]) == -1) { flag = + 1; } }
				if(cluster[coord[0]][coord[1]][coord[2]] == 0) {
					...
					return (clusterize(latt, cluster, boundary, l , t, beta, coord[0], coord[1], coord[2]) || flag);
				}
...
int surfaceCluster(int*** latt, int boundary, unsigned int l , unsigned int t, long double beta) {
	...
	//FIXME: L'algoritmo non funziona quando il cluster si avvolge attorno al reticolo senza toccarsi (è improbabile), posso aggingere un controllo che permette il cross una sola volta per ogni cluster.
