...
\item The only requirement on the update step is that all the possible $\lbrace s \rbrace$ given $\lbrace \sigma \rbrace$ which we choose to have a non null probability of happearing are indeed equiprobable.
\item We can also update the coupling constants $J_{\langle x, y \rangle}$ as long as $J_{\langle x, y \rangle} s_x s_y$ where $\sigma = 1$.
