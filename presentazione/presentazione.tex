...
\item The only requirement on the update step is that all the possible $\lbrace s \rbrace$ given $\lbrace \sigma \rbrace$ which we choose to have a non null probability of happearing are equiprobable.
...
\vspace{10pt}
In the Wolff algorithm we choose at random one cluster and flip it with certainty.\\
\vspace{10pt}
It's equivalent to choosing at random one site of the lattice and considering it to be the seed for and expanding cluster and then flipping it.  
