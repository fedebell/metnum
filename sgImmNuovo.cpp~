...
int main(int argc, char *argv[]) {
	...
	for(int i = 0; i < Beta.size(); i++) {
		...
		for(int j = 0; j < L.size(); j++) {
			...
			file.open ("dataImm" + beta_str.str() + ":" + l1_str.str() + ".txt");
			...
			cout << beta << "\t" << l1 << "\t" << result.value << "\t" << result.error << endl << flush;
			
			//file << beta << "\t" << l1 << "\t" << result.value << "\t" << result.error << endl << flush;
...
F_mod jackknife(int* boundary, int size, int* blockDimentions, int len, int t) {
	...
	double media = 0.0, sum = 0.0, F_biased = 0.0, temp = 0.0;
	...
	for(i = 0; i < len; i++) {
		...
		F_unbiased[i] = Nb * F_biased - (((Nb - 1) * blockDimentions[i])/(Nb * blockDimentions[i])) * media;
			
		media /= Nb;
		
		cout << F_unbiased[i] << "\t" << F_biased << "\t" << media << endl;
