...
#blocking 
while i < numpy.sqrt(len(mag_per)): #scorro sui posssibli divisori del numero di eventi
	#creo array i cui elementi sono le medie dei blocchetti desiderati
	n.append(i) #salvo la dimensione dei blocchi
	block = []
	avg = totsum
	for cnt in range(len(mag_per) - (len(mag_per)//i) * i):
		avg -= mag_per[(len(mag_per)//i) * i + cnt] #tolgo gli elementi in eccesso ,che comunque non vengono passati dal for successivo...
	avg = avg / (i* (len(mag_per)//i) ) #true avg	
